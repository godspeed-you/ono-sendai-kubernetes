...
**v0.2.3 ships the package as a `.deb` and an `.rpm`, signed by nobody's key**: the release
...
| Releases of this provider | **v0.2.3** — the signed `.deb`, `.rpm` and `.kuang`, needing `ono >= 0.4.4`; v0.2.2's `.kuang` was packed without its signature and should not be used; v0.2.0 brought the KUANG/11 permission contract; v0.1.0 was the first, built from this repository as a KUANG/11 package |
