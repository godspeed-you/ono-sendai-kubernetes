...
fn observe(
    ...
    let (resource, listing) = match acquired {
...
fn live(
    ...
    if let Step::Stopped(outcome) = acquisition(lease, held, emitter, &gvr, &scope, &freshness) {
        return outcome;
    }
    ...
    loop {
        ...
        let from = held.with(|session| {
            session.watch_stream(&gvr, &scope).and_then(|stream| {
                stream
                    .checkpoint()
